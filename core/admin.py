...
from .views import download_tag_template, preview_tag_import
# =================================================================
# 1. SHARED ACTIONS
# =================================================================
...
# 1. Provide initial data so the form doesn't see 'store' as empty
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('import-modisoft/', self.admin_site.admin_view(self.import_modisoft), name='import-modisoft'),
        ]
        return custom_urls + urls

# - Start import of Modisoft file----------------------------------------------------
    def import_modisoft(self, request):
        # ---------------------------------------------------------
        # HELPER: PROCESS FILE (Shared by Preview and Save phases)
        # ---------------------------------------------------------
        def process_import_file(file_path, active_store, commit=False):
            results = {
                'new': [],       
                'update': [],    
                'rejected': [],  
            ...
            try:
                ...
                # 2. MAP COLUMNS
                try:
                    if 'scan code' in header_map:
                        sku_idx = header_map['scan code']
                    else:
                        raise ValueError("Scan code")

                    if 'item description' in header_map:
                        name_idx = header_map['item description']
                    else:
                        raise ValueError("Item description")

                    if 'unit price' in header_map:
                        price_idx = header_map['unit price']
                    elif 'unit retail' in header_map:
                        price_idx = header_map['unit retail']
                    else:
                        raise ValueError("Unit price")

                except ValueError as e:
                    return None, f"Missing required column: {str(e)}"
                ...
                for row_idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                    ...
                    if not raw_sku or not raw_name or not raw_price:
                        results['rejected'].append({
                            'row': row_idx, 'sku': raw_sku or "MISSING", 
                            'reason': "Missing Scan code, Name, or Price"
                        })
                        continue

                    # Validation 2: Duplicates
                    if raw_sku in seen_skus_in_file:
                        results['rejected'].append({
                            'row': row_idx, 'sku': raw_sku, 
                            'reason': "Duplicate SKU in file"
                        })
                        continue
                    seen_skus_in_file.add(raw_sku)

                    # Validation 3: Price Format
                    try:
                    ...
                    # DB Logic
                    existing_product = Product.objects.filter(sku=raw_sku, store=active_store).first()

                    if existing_product:
                        # UPDATE EXISTING
                        current_db_price = existing_product.price.quantize(Decimal("0.00"))
                        
                        # CHANGE: Check if price OR name has changed
                        price_changed = current_db_price != price_decimal
                        name_changed = existing_product.name != raw_name
                        ...
                        if price_changed or name_changed:
                            item_data = {
                                ...
                                'old_name': existing_product.name, # Optional: display in table
                            }
                            results['update'].append(item_data)
                            
                            if commit:
                                existing_product.name = raw_name
                        ...
                        # CREATE NEW
                        item_data = {
                            'sku': raw_sku, 'name': raw_name, 'new_price': price_decimal
                        }
                        results['new'].append(item_data)
                        
                        if commit:
                            # FIXED: Removed 'company=...' argument
        ...
        if request.method == "POST" and "confirm_save" in request.POST:
            ...
            if not temp_file_path or not os.path.exists(temp_file_path):
                self.message_user(request, "Session expired. Please upload again.", messages.ERROR)
                return redirect("..")
                
            results, error = process_import_file(temp_file_path, request.active_store, commit=True)
            
            if error:
                self.message_user(request, error, messages.ERROR)
            else:
                os.remove(temp_file_path)
                msg = f"Import Complete! New: {len(results['new'])}, Updated: {len(results['update'])}"
        ...
        if request.method == "POST" and request.FILES.get("import_file"):
            ...
            filename = default_storage.save(f'tmp/{import_file.name}', import_file)
            temp_file_path = os.path.join(settings.MEDIA_ROOT, filename)
            
            results, error = process_import_file(temp_file_path, request.active_store, commit=False)
            ...
            return render(request, "admin/core/product/import_preview.html", {
                "title": f"Import Preview for {request.active_store.name}",
                "results": results,
                "temp_file_path": temp_file_path,
                "store": request.active_store
            })

        # ---------------------------------------------------------
        # PHASE 1: UPLOAD FORM
        # ---------------------------------------------------------
        return render(request, "admin/core/product/import_upload.html", {
            "title": "Upload Modisoft XLSX",
            "store": request.active_store
        })

# - End import of Modisoft file----------------------------------------------------

...
class ESLTagUniquenessTest(TestCase):
    ...
    def test_same_tag_different_stores(self):
        ...
        # MACs are normalized to uppercase in model save()
        mac = "390000F41F5F"
        tag1 = ESLTag.objects.create(tag_mac=mac, gateway=self.gw1, hardware_spec=self.hw)
        tag2 = ESLTag.objects.create(tag_mac=mac, gateway=self.gw2, hardware_spec=self.hw)

        self.assertEqual(tag1.store, self.store1)
        self.assertEqual(tag2.store, self.store2)
        self.assertEqual(ESLTag.objects.filter(tag_mac=mac).count(), 2)

    def test_same_tag_same_store_fails(self):
        """Verify that the same tag_mac cannot exist twice in the same store."""
        mac = "390000F41F5F"
        ESLTag.objects.create(tag_mac=mac, gateway=self.gw1, hardware_spec=self.hw)

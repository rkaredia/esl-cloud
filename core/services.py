        ...
        if commit:
            with transaction.atomic():
                ...
                if updated_skus:
                    ...
                    # PERFORMANCE: Evaluating the QuerySet into a list here ensures that we perform
                    # a single O(N) database query and can then perform a single O(1) batch
                    # dispatch of background tasks via Celery groups, rather than queuing
                    # N individual tasks in a loop.
                    ...
                    if tag_ids:
                        # Batch dispatch tasks AFTER the DB transaction is successful

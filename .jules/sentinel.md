...
## 2026-06-04 - Enforcing Store-Level Isolation for All Staff Roles
**Vulnerability:** The `CompanySecurityMixin.get_queryset` logic only applied store-level isolation for the 'manager' role. Other roles with staff access, such as 'staff' and 'readonly', were able to see data across all stores within their company, violating the principle of least privilege.
**Learning:** Security mixins must account for all roles that possess administrative access (`is_staff=True`). Partial coverage in isolation logic creates gaps where lower-privileged roles can access data reserved for specific store assignments. Additionally, filtering `ESLTag` through a join on `gateway` can inadvertently hide tags without hardware pairings; filtering by the `store` field directly is more robust and secure.
**Prevention:** Ensure store-level isolation logic in `get_queryset` covers all restricted roles ('manager', 'staff', 'readonly'). Use direct foreign key filtering (e.g., `store__in`) whenever possible to avoid data leakage caused by missing related records in joins.

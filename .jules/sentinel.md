...
## 2026-05-21 - Securing Custom Admin Views and Fixing User Admin MRO
**Vulnerability:** Custom administrative views in SAISAdminSite (Template Gallery/Design Lab) lacked explicit role-based checks, allowing 'Read-Only' staff users to access hardware debugging tools. Additionally, CustomUserAdmin inheritance order (MRO) caused security mixins to be shadowed by the base UserAdmin class.
**Learning:** Custom URLs added to an AdminSite via get_urls() are protected by admin_view, which ensures is_staff, but does NOT enforce granular roles or permissions by default. Furthermore, when extending standard Django Admin classes like UserAdmin, security mixins must be listed FIRST in the inheritance list to ensure their overrides (like get_queryset or save_model) are prioritized in the Method Resolution Order.
**Prevention:** Always implement explicit role checks (request.user.role) or permission checks (has_perm) within custom admin views. Verify MRO for all Admin classes using security mixins by ensuring the mixin precedes the base admin class.

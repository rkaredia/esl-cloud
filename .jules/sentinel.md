...
## 2026-05-12 - Closing Store-Level RBAC Gaps for Restricted Roles
**Vulnerability:** Staff and Read-only users were able to bypass store-level data isolation because `CompanySecurityMixin` only applied filtering for the 'manager' role. Additionally, staff users could access hardware debugging views (Template Design Lab) which should be restricted to managers and above.
**Learning:** Security mixins that enforce row-level isolation (RLS) must explicitly include all roles that require restriction. Relying on a single 'manager' check leaves other restricted roles (like staff) with elevated visibility. Custom Admin views that bypass standard model-level permissions require explicit hierarchical role checks.
**Prevention:** Audit all security mixins to ensure every restricted role is included in isolation logic. Implement explicit `request.user.role` checks in custom views to ensure only appropriate levels of management can access technical debugging tools.

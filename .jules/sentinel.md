...
## 2026-05-15 - Securing Custom Admin Views via Explicit Role Checks
**Vulnerability:** Custom administrative views registered via `SAISAdminSite` (specifically `template_gallery` and `mock_render_view`) lacked explicit role-based access control. Any user with `is_staff` access (including 'staff' and 'readonly' roles) could access hardware debugging and design tools by entering the URL directly.
**Learning:** Custom views added to `AdminSite.get_urls()` are protected by `admin_view`, which ensures `is_staff`, but they do not automatically inherit the row-level security or role-based restrictions applied to `ModelAdmin` classes. Granular role-based checks (e.g., restricting to superusers, owners, or managers) must be explicitly implemented within the view function.
**Prevention:** Always implement explicit role or permission checks in custom administrative views. Raise `django.core.exceptions.PermissionDenied` if the user's role does not meet the minimum requirements for the tool or data being accessed.

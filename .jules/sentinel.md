...
## 2026-05-15 - Restricting Admin Utility Access via Explicit Role Checks
**Vulnerability:** Administrative utility views like the 'Design Lab' (template gallery and mock rendering) were accessible to all staff users (is_staff=True), regardless of their specific role (e.g., 'staff' vs 'manager'). This exposed hardware debugging tools and rendering logic to lower-privileged operational accounts.
**Learning:** Django Admin's 'admin_view' wrapper only enforces 'is_staff' by default. Custom views added to 'SAISAdminSite' that don't correspond to a specific model must implement explicit role-based or permission-based checks within the view function to prevent unintended access by limited staff accounts.
**Prevention:** Always verify 'request.user.role' or specific granular permissions within custom administrative views. Use 'PermissionDenied' to fail securely when unauthorized access is detected.
